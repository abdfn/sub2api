...
// TestSettingsCodexTicketProxyWriteReadAndHotReload 验证 Settings Codex Ticket Proxy Write Read And Hot Reload 场景
...
// TestSettingsCodexTicketRejectInvalidProxyWithoutLeakingPassword 验证 Settings Codex Ticket Reject Invalid Proxy Without Leaking Password 场景
...
// TestSettingsCodexTicketRejectEnableWithoutProxy 验证缺少代理时不能保存启用状态
func TestSettingsCodexTicketRejectEnableWithoutProxy(t *testing.T) {
	key := service.SettingKeyOpenAICodexTicketEnabled
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{key: "false"})
	rec := doUpdateSettings(t, h, map[string]any{key: true}, nil)
	require.Equal(t, http.StatusBadRequest, rec.Code, rec.Body.String())
	require.Contains(t, rec.Body.String(), "INVALID_CODEX_HARVEST_PROXY")
