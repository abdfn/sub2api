...
type openAICodexTicketHarvestPauseContextKey struct{}

// openAICodexTicketHarvestPausedContext 优先通过上下文中的服务读取最新额度暂停状态
func openAICodexTicketHarvestPausedContext(ctx context.Context, account *Account, now time.Time) bool {
	if ctx != nil {
		if service, ok := ctx.Value(openAICodexTicketHarvestPauseContextKey{}).(*OpenAIGatewayService); ok {
			return service.openAICodexTicketHarvestPaused(account, now)
		}
	}
	return openAICodexTicketHarvestPaused(account, now)
}

// parseOpenAICodexTicketHarvestPause 读取账号持久化的采票额度暂停标记
...
// active 结合额度窗口和快照新鲜度判断采票暂停是否仍有效
...
// openAICodexTicketHarvestPaused 判断账号是否因额度耗尽暂停后台采票
func openAICodexTicketHarvestPaused(account *Account, now time.Time) bool {
	return parseOpenAICodexTicketHarvestPause(account).active(account, now)
}
...
func (s *OpenAIGatewayService) openAICodexTicketHarvestPaused(account *Account, now time.Time) bool {
	...
	if s != nil && account != nil {
		if raw, ok := s.openaiCodexTicketQuotaPauses.Load(account.ID); ok {
			...
			if pause == nil || local.ObservedAt.After(pause.ObservedAt) {
				pause = local
			}
		}
	}
...
// pauseOpenAICodexTicketHarvestOnQuota 根据限流响应和可信额度快照记录采票暂停
