...
// TestCodexTicketSettingsRequireEffectiveProxy 验证启用打票必须具有有效代理，并支持启动配置回退
func TestCodexTicketSettingsRequireEffectiveProxy(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "disabled"},
		{name: "missing", enabled: true, proxy: "  ", wantError: true},
		{name: "configured", enabled: true, proxy: "http://proxy.example:8080"},
		{name: "fallback", enabled: true, fallback: "socks5h://proxy.example:1080"},
		{name: "invalid fallback", enabled: true, fallback: "ftp://proxy.example", wantError: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			svc := &SettingService{cfg: &config.Config{Gateway: config.GatewayConfig{
				OpenAICodexTicket: config.OpenAICodexTicketConfig{HarvestProxyURL: tc.fallback},
			}}}
			err := svc.validateOpenAICodexTicketSettings(&SystemSettings{
				OpenAICodexTicketEnabled: tc.enabled, OpenAICodexTicketHarvestProxyURL: tc.proxy,
			})
			if tc.wantError {
				require.Error(t, err)
			} else {
...
// GetValue 为打票回归测试提供 Get Value 辅助实现
...
// TestCodexTicketEnabledRuntimeSettingOverridesYaml 验证 Codex Ticket Enabled Runtime Setting Overrides Yaml 场景
...
// TestRefreshOpenAICodexTickets_DisabledSkipsHarvest 验证 Refresh Open AICodex Tickets Disabled Skips Harvest 场景
...
// TestCodexTicketProxyRuntimeSettingAndFallback 验证 Codex Ticket Proxy Runtime Setting And Fallback 场景
...
// TestCodexTicketProxyMaskAndValidation 验证 Codex Ticket Proxy Mask And Validation 场景
...
// TestCodexTicketSettingsRefreshDoesNotMutateSharedConfig 验证 Codex Ticket Settings Refresh Does Not Mutate Shared Config 场景
