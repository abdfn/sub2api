...
// validateOpenAICodexTicketSettings 校验开启打票时的有效代理，允许回退到启动配置
func (s *SettingService) validateOpenAICodexTicketSettings(settings *SystemSettings) error {
	if !settings.OpenAICodexTicketEnabled {
		return nil
	}
	proxy := strings.TrimSpace(settings.OpenAICodexTicketHarvestProxyURL)
	if proxy == "" && s != nil && s.cfg != nil {
		proxy = strings.TrimSpace(s.cfg.Gateway.OpenAICodexTicket.HarvestProxyURL)
	}
	if proxy == "" {
		return errors.New("开启 Codex 打票时必须配置打票代理")
	}
	return ValidateOpenAICodexTicketHarvestProxyURL(proxy)
}

const openAICodexTicketEnabledCacheTTL = 5 * time.Second

// GetOpenAICodexTicketEnabled 返回后台 Codex 门票总开关（个人 292 / Team 332）。
...
// InvalidateOpenAICodexTicketEnabledCache 使打票开关缓存失效以便重新读取后台设置
...
// GetOpenAICodexTicketHarvestProxyURL 返回后台配置的 Codex 门票代理。空则调用方回退 yaml/env。
...
// InvalidateOpenAICodexTicketHarvestProxyCache 使打票代理缓存失效以便后续尝试使用新配置
